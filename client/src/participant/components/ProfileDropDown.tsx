...
return (
  <div className="relative font-orbitron" ref={dropdownRef} style={{ fontFamily: "'Orbitron', sans-serif" }}>
    ...
    <button
      ...
      className="flex items-center justify-between sm:justify-start gap-2 px-3 py-2 rounded-lg bg-[#2a0a46]/60 hover:bg-[#3a0a6b]/70 border border-purple-700 text-purple-200 transition-all duration-200 w-full sm:w-auto"
    >
      <div className="w-8 h-8 rounded-full bg-gradient-to-br from-purple-500 to-cyan-600 flex items-center justify-center shrink-0">
        <span className="text-sm font-bold text-white">
          {user.name.charAt(0).toUpperCase()}
        </span>
      </div>

      {/* Hide Name + Role on xs screens */}
    ...
    {/* Dropdown */}
    {isOpen && (
      <div className="absolute right-0 mt-2 w-[90vw] sm:w-80 max-w-sm bg-[#1b052f]/95 border border-purple-800 rounded-xl shadow-[0_0_20px_rgba(130,50,255,0.3)] overflow-hidden z-50 backdrop-blur-md">
        ...
        <div className="p-4 bg-[#2b0a4a]/70 border-b border-purple-700">
          ...
          <div className="text-sm text-purple-300 flex items-center gap-2 break-all">
        ...
        {team ? (
          <div className="p-4 border-b border-purple-700">
            ...
            <div className="bg-[#25083f]/60 rounded-lg p-3 space-y-2 max-h-40 overflow-y-auto">
              <div className="flex justify-between items-center">
                <span className="font-semibold text-purple-100 truncate">{team.team_name}</span>
                <span className="flex items-center gap-1 text-cyan-300">
                  <Trophy className="w-4 h-4" /> {team.points}
                </span>
              </div>

              <div className="text-xs text-purple-400">{team.members.length} members</div>
          ...
          <div className="p-4 border-b border-purple-700 text-purple-400 text-sm flex items-center gap-2">
            <Users className="w-4 h-4" /> Not in a team
          </div>
        )}
        ...
        <button
          ...
          disabled={loggingOut}
          className="w-full p-4 flex items-center gap-3 text-pink-400 hover:bg-pink-600/10 transition-colors duration-200 disabled:opacity-50"
        >
