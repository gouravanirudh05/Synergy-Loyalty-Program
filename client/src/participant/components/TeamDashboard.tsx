...
const TeamDashboard: React.FC<TeamDashboardProps> = ({ team, onTeamLeft }) => {
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 sm:px-6 py-6 font-['Orbitron'] space-y-6">
      {/* Header */}
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 text-center sm:text-left">
        ...
        <button
          ...
          disabled={leaving}
          className="mx-auto sm:mx-0 flex items-center justify-center gap-2 px-4 py-2 border border-pink-500/60 text-pink-400 hover:bg-pink-500/10 rounded-lg transition-all duration-300 disabled:opacity-50 text-sm sm:text-base"
        >
          ...
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div className="flex-1 text-center md:text-left">
              ...
              <p className="text-slate-400 text-sm mb-3">
                ...
                {3 - team.members.length > 1 ? "s" : ""} available. Share this code.
              </p>
              <div className="flex flex-col sm:flex-row gap-3">
                <div className="flex-1 px-4 py-3 bg-slate-800/60 border border-purple-500/30 rounded-lg">
                  ...
                  <div className="font-mono text-2xl text-purple-300 font-bold tracking-widest break-all">
                    {team.join_code || "N/A"}
                  </div>
                </div>
                <button
        ...
        <div className="bg-slate-900/50 border border-cyan-500/30 rounded-xl p-6 shadow-[0_0_15px_rgba(34,211,238,0.15)]">
          ...
          <div className="flex flex-col items-center">
            <div className="bg-white p-3 sm:p-4 rounded-xl mb-4">
              <QRCodeSVG value={team.qr_id || team.team_id} size={150} />
            </div>
      ...
      <div className="bg-slate-900/50 border border-purple-500/30 rounded-xl p-6 shadow-[0_0_15px_rgba(168,85,247,0.15)]">
        ...
        <div className="space-y-3">
          {team.members.map((member, index) => (
            <div
              key={index}
              className="flex flex-wrap items-center justify-between gap-3 p-3 bg-slate-800/40 rounded-lg border border-slate-700/40"
            >
